...
def compare(img_path: str, binary: list[cv2.UMat], threshold: float, mask: list[cv2.UMat]) -> bool:
    img = cv2.UMat(cv2.imread(img_path, cv2.IMREAD_GRAYSCALE))
    white_pixels = cv2.countNonZero(img)
    if white_pixels < 300:
        return False
    part_max: list[float] = []
    for image in zip(binary, mask):
        res = cv2.matchTemplate(img, image[0], cv2.TM_CCORR_NORMED, mask=image[1]).get()
        res[np.isinf(res)] = 0
